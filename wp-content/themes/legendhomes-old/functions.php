<?php
// Load only one version of jquery smc
...
function remcat_rewrite($wp_rewrite) {
    ...
    $new_rules = array('(.+)/(?:feed/)?(feed|rdf|rss|rss2|atom)/?' => 'index.php?category_name=$matches[1]&feed=$matches[2]');
    $wp_rewrite->rules = $new_rules + $wp_rewrite->rules;
}

// Wordpress: Removes Header Links
...
include_once(TEMPLATEPATH.'/themeoptions.php');
...
add_filter('single_template', 'custom_single');
function custom_single() {

$category = get_the_category();
$cat_parent_ID = $category[0]->parent;

	if ($cat_parent_ID == 119) {
		$template = locate_template( array( 'single-home.php', 'single.php' ) );	
	} else if ($cat_parent_ID == 7) {
		$template = locate_template( array( 'single-news.php', 'single.php' ) );	
	} else {		
...
function custom_category() {
	global $lh_news_events;
	$lhn_cats=explode(',',$lh_news_events);

	$category = get_the_category();
	$catID = $category[0]->cat_ID;	

	if (in_array($catID, $lhn_cats)) {
		$template = locate_template( array( 'category-news-events.php', 'category.php' ) );
	} else {
...
// Returns the community based on the URL - not Category IDs
function get_community(){
	$purl = explode("/", $_SERVER["REQUEST_URI"]);
	foreach ($purl as $value){
		     if (strpos($value,'edgewater') !== false) 	{$community = "edgewater";}
		else if (strpos($value,'villebois') !== false) 	{$community = "legend-at-villebois";}
		else if (strpos($value,'orenco') !== false) 		{$community = "village-at-orenco";}
		else if (strpos($value,'walnut') !== false) 		{$community = "walnut-creek";}
		else if (strpos($value,'willamette') !== false) {$community = "willamette-landing";}
		else if (strpos($value,'tryon-highlands') !== false) {$community = "tryon-highlands";}			
		else if (strpos($value,'move-in-ready') !== false) {$community = "move-in-ready";}
	}
return $community;	
}


/*realtor section stuff */

add_filter('the_password_form', 'get_lh_the_password_form');
add_filter('protected_title_format', 'get_lh_protected_title');

function get_lh_the_password_form() {
	global $post;
	$label = 'pwbox-'.(empty($post->ID) ? rand() : $post->ID);
	$content = '<style>#primary {display: none;}</style><div id="login-big"><div id="login-floater"><p class="green">' . __('Don\'t have a realtor login? <a id="orangebox" href="/realtor-sign-up">Create New Account</a>') . '</p><p class="spacerbox"><h3>Realtor Sign In</h3></p><form action="' . get_option('siteurl') . '/wp-pass.php" method="post">
	
	<p class="spacerbox"><label for="loginname">Login<input id="loginname" type="text" class="inputspace" /></label><br /><label for="' . $label . '">' . __("Password:") . ' <input name="post_password" id="' . $label . '" type="password" size="20" class="inputspace" /></label> <input type="submit" name="Submit" class="submitlogin" value="' . esc_attr__("Submit") . '" /></p>
	</form><p class="spacerbox"><a href="/forgot-password">Forgot password?</a></div></div>	
	';
	return $content;
}


function get_lh_protected_title() {
	$title = '';
	return $title;
}
...
//expects to get called inside THE LOOP
function homepage_excerpt(){
    ob_start();
    // write content
    the_excerpt();
	...
	echo implode(' ',array_slice($acontent,0,20)).( count($acontent) != count($content) ? '&hellip;' : '' );
}

function sidebar_excerpt(){
    ob_start();
    // write content
    the_excerpt();
    $content = ob_get_contents();
...
function get_category_siblings($thecat = '') { //expects you to pass it the result of get_category
	$parent = $thecat->category_parent;
	$thischildid = $thecat->cat_ID;
	echo '<ul>';
	wp_list_categories('exclude=1&orderby=ID&show_count=0&title_li=&use_desc_for_title=0&child_of='.$parent.'&depth=1');
	echo '</ul>';
}
...
function lh_globalnav() {
	if ( $menu = str_replace( array( "\r", "\n", "\t" ), '', wp_list_categories('title_li=&orderby=ID&echo=0&depth=2&exclude=1&use_desc_for_title=0') ) )
		$menu = '<ul class="nav">' . $menu . '</ul>';
	$menu = '<div id="menu">' . $menu . "</div>\n";
	echo apply_filters( 'globalnav_menu', $menu ); // Filter to override default globalnav: globalnav_menu
}

function lh_footernav() {
	if ( $menu = str_replace( array( "\r", "\n", "\t" ), '', wp_list_categories('title_li=&orderby=ID&echo=0&depth=2&exclude=1&use_desc_for_title=0') ) )
		$menu = '<ul class="foot">' . $menu . '</ul>';
	//$menu = '<div id="menu">' . $menu . "</div>\n";
	//echo apply_filters( 'globalnav_menu', $menu ); // Filter to override default globalnav: globalnav_menu
	echo $menu;
}
...
function footernav() {
	if ( $menu = str_replace( array( "\r", "\n", "\t" ), '', wp_list_categories('title_li=&orderby=ID&echo=0&depth=1&exclude=1&use_desc_for_title=0') ) )
		$menu = '<ul>' . $menu . '</ul>';
...
function getGall($dir) {
		...
		$fulldir = "{$_SERVER['DOCUMENT_ROOT']}/uploads/$dir"; 	
		$d = @dir($fulldir) or die("getImages: Failed opening directory $dir for reading"); 
		
		while(false !== ($entry = $d->read())) { 
			$size = getimagesize("$fulldir$entry");
			$path = "/$dir$entry";			
			if ($entry[0] == "." || ($size['mime'] != 'image/jpeg')) continue;			
			$retval[] = array( "file" => $path, "size" => $size );			
		}
...
function lf1() {
    return '<!-- Google Code for lead form filled out Conversion Page -->
...
var google_conversion_label = "pf5_CLbIlAIQ6uKe7QM";
var google_conversion_value = 0;
if (300000) {
  google_conversion_value = 300000;
}
/* ]]> */
</script>
...
/* shortcode for vidoe tracking behind our walls playlist (start) */

function vpwalls( $atts ){
 include 'wallvideos.php';
}
add_shortcode( 'wallvideos', 'vpwalls' );
...
function register_theme_sidebars() {
   ...
   register_sidebar( array(
	   'before_widget' => '<div id="youtube">',
	   'after_widget' => '</div>',
	   'before_title' => '<div class="tubehead"><h4>',
	   'after_title' => '</h4></div>',
	   'name' => 'Home YouTube',
	   'id' => 'homeyoutube'
   ));
